...
export const CourseView = () => {
  ...
  if (!course) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/10 flex items-center justify-center p-4 sm:p-6">
              ...
              <Card className="glass-card border-white/10 shadow-2xl backdrop-blur-xl">
                <CardContent className="p-6 sm:p-8 lg:p-12">
                  ...
                  <div className="relative mb-6 sm:mb-8">
                    <div className="w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 mx-auto bg-gradient-to-br from-red-500/20 to-orange-500/20 rounded-full flex items-center justify-center border border-red-500/30">
                      <BookOpen className="h-8 w-8 sm:h-10 sm:w-10 lg:h-12 lg:w-12 text-red-400" />
                    </div>
                    <div className="absolute -top-1 -right-1 sm:-top-2 sm:-right-2 w-6 h-6 sm:w-8 sm:h-8 bg-gradient-to-br from-red-500 to-orange-500 rounded-full flex items-center justify-center">
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold bg-gradient-to-r from-red-400 via-orange-400 to-red-500 bg-clip-text text-transparent">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-6 sm:mt-8 justify-center">
                    <Button 
                      ...
                      className="bg-gradient-to-r from-primary to-secondary hover:from-primary/90 hover:to-secondary/90 shadow-lg hover:shadow-xl transition-all duration-200 text-sm sm:text-base"
                    >
                    ...
                    <Button 
                      ...
                      className="border-primary/20 text-primary hover:bg-primary/10 transition-all duration-200 text-sm sm:text-base"
                    >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            <Card className="glass-card border-blue-500/20 bg-blue-500/5">
              <CardContent className="p-3 sm:p-4 text-center">
                <Search className="h-5 w-5 sm:h-6 sm:w-6 mx-auto mb-2 text-blue-400" />
                <h3 className="font-medium text-xs sm:text-sm">{t('studentCourseView.checkUrl')}</h3>
            ...
            <Card className="glass-card border-primary-500/20 bg-primary-500/5">
              <CardContent className="p-3 sm:p-4 text-center">
                <BookOpen className="h-5 w-5 sm:h-6 sm:w-6 mx-auto mb-2 text-primary-400" />
                <h3 className="font-medium text-xs sm:text-sm">{t('studentCourseView.browseAvailable')}</h3>
                <p className="text-xs text-muted-foreground">{t('studentCourseView.browseAvailableDescription')}</p>
              </CardContent>
            </Card>
            
            <Card className="glass-card border-purple-500/20 bg-purple-500/5 sm:col-span-2 lg:col-span-1">
              <CardContent className="p-3 sm:p-4 text-center">
                <HelpCircle className="h-5 w-5 sm:h-6 sm:w-6 mx-auto mb-2 text-purple-400" />
                <h3 className="font-medium text-xs sm:text-sm">{t('studentCourseView.needHelp')}</h3>
            ...
            <div className="lg:col-span-2 space-y-6 sm:space-y-8">
              ...
              <div className="space-y-4 sm:space-y-6">
                <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                  ...
                  { course?.category && 
                  <Badge variant="default" className="bg-primary/10 text-primary border-primary/20 px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm">
                    {course.category}
                  </Badge>
                  }
                ...
                <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-6xl font-bold bg-gradient-to-r from-foreground via-foreground/90 to-foreground/70 bg-clip-text text-transparent leading-tight">
                  {course.title}
                </h1>
                ...
                {/* Course Highlights */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                  <div className="flex items-center gap-2 sm:gap-3 p-3 sm:p-4 rounded-xl bg-gradient-to-r from-primary-500/10 to-secondary-500/10 border border-primary-500/20">
                    <PlayCircle className="h-6 w-6 sm:h-8 sm:w-8 text-primary-500 flex-shrink-0" />
                    <div className="min-w-0 flex-1">
                      <div className="font-semibold text-sm sm:text-base">{progress.totalLessons} {t('studentCourseView.lessons')}, {progress.totalQuizzes} {t('studentCourseView.quizzes')}</div>
                      <div className="text-xs text-muted-foreground">{t('studentCourseView.highQualityContent')}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 sm:gap-3 p-3 sm:p-4 rounded-xl bg-gradient-to-r from-blue-500/10 to-accent-500/10 border border-blue-500/20">
                    <Clock className="h-6 w-6 sm:h-8 sm:w-8 text-blue-500 flex-shrink-0" />
                    <div className="min-w-0 flex-1">
                      <div className="font-semibold text-sm sm:text-base">{Math.round(totalCourseMinutes / 60) > 0 ? `${Math.round(totalCourseMinutes / 60)}+ ${t('studentCourseView.hours')}` : `${totalCourseMinutes} ${t('studentCourseView.minutes')}`}</div>
                      <div className="text-xs text-muted-foreground">{t('studentCourseView.onDemandVideo')}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 sm:gap-3 p-3 sm:p-4 rounded-xl bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-500/20 sm:col-span-2 lg:col-span-1">
                    <Infinity className="h-6 w-6 sm:h-8 sm:w-8 text-purple-500 flex-shrink-0" />
                    <div className="min-w-0 flex-1">
                      <div className="font-semibold text-sm sm:text-base">{t('studentCourseView.lifetime')}</div>
                    ...
                    <CardContent className="p-4 sm:p-6">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <h3 className="font-semibold text-primary-700 dark:text-primary-300 text-sm sm:text-base">{t('studentCourseView.yourProgress')}</h3>
                            <p className="text-xs sm:text-sm text-primary-600 dark:text-primary-400">{t('studentCourseView.keepUpGreatWork')}</p>
                          </div>
                        </div>
                      ...
                      <div className="w-full bg-primary-100 dark:bg-primary-900/40 rounded-full h-2 sm:h-3 mb-2">
                        <div 
                          className="bg-gradient-to-r from-primary-500 to-primary-500 h-2 sm:h-3 rounded-full transition-all duration-700 ease-out"
              ...
              {/* Mobile Enroll Sidebar - Visible on small screens up to lg */}
                ...
                <Card className="glass-card border-white/10 shadow-2xl">
                  <CardContent className="p-4 sm:p-6 space-y-4 sm:space-y-6">
                    ...
                    {course.cover_image_url && (
                      <div className="text-center">
                        <img 
                          ...
                          alt={course.title}
                          className="w-full max-w-xs mx-auto rounded-lg object-cover border border-border/20 shadow-lg"
                        />
                      </div>
                    )}
                    ...
                    <div className="text-center space-y-2">
                      ...
                      <p className="text-xs sm:text-sm text-muted-foreground">{t('studentCourseView.oneTimePurchase')}</p>
                    </div>
                    ...
                    {!isEnrolled ? (
                      ...
                      <div className="space-y-3 sm:space-y-4">
                        <div className="flex items-center justify-center gap-2 text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/20 p-3 sm:p-4 rounded-xl">
                          ...
                          <span className="font-semibold text-sm sm:text-base">{t('studentCourseView.youreEnrolled')}</span>
                        </div>
                        <Button asChild className="w-full h-12 sm:h-14 text-base sm:text-lg font-semibold rounded-xl">
                    ...
                    {/* Course Features */}
                    <div className="space-y-3 sm:space-y-4 pt-3 sm:pt-4 border-t border-border/20">
                      <h4 className="font-semibold text-sm sm:text-base text-center">{t('studentCourseView.thisCourseIncludes')}</h4>
                    ...
                    {/* Share */}
                    <div className="flex gap-2 pt-3 sm:pt-4 border-t border-border/20">

                      <Button variant="outline" size="sm" className="flex-1 text-xs sm:text-sm" onClick={handleShareClick}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                    {[
                      ...
                      <div key={index} className="flex items-start gap-2 sm:gap-3">
                        <CheckCircle className="h-4 w-4 sm:h-5 sm:w-5 text-primary-500 mt-0.5 flex-shrink-0" />
              ...
              <Card className="glass-card border-white/10">
                <CardHeader className="pb-3 sm:pb-6">
                  <CardTitle className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 text-lg sm:text-xl">
                    ...
                    <Badge variant="default" className="w-fit sm:ml-auto text-xs sm:text-sm">
                ...
                <CardContent className="pt-0">
                  {lessons.length === 0 ? (
                    <div className="text-center py-8 sm:py-12 text-muted-foreground">
                      ...
                      <h3 className="text-base sm:text-lg font-medium mb-2">{t('studentCourseView.noLessonsYet')}</h3>
                      <p className="text-sm sm:text-base">{t('studentCourseView.instructorStillPreparing')}</p>
                    </div>
                  ) : (
                    <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                      <TabsList className="grid w-full grid-cols-3">
                        <TabsTrigger value="lessons" className="text-xs sm:text-sm px-2 sm:px-4">{t('studentCourseView.lessons')}</TabsTrigger>
                        <TabsTrigger value="quizzes" className="text-xs sm:text-sm px-2 sm:px-4">{t('studentCourseView.quizzes')}</TabsTrigger>
                        <TabsTrigger value="lectures" className="text-xs sm:text-sm px-2 sm:px-4">{t('studentCourseView.liveLectures')}</TabsTrigger>
                      </TabsList>

                      <TabsContent value="lessons" className="space-y-4 sm:space-y-6 mt-4 sm:mt-6">
                        <Accordion type="single" collapsible className="w-full space-y-2">
                          {lessons.map((lesson: Lesson) => (
                            <AccordionItem key={lesson.id} value={lesson.id} className="border border-border/50 rounded-lg overflow-hidden bg-muted/20">
                              <AccordionTrigger className="hover:bg-muted/50 px-3 sm:px-6 py-3 sm:py-4 hover:no-underline">
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between w-full mr-2 sm:mr-4 gap-2 sm:gap-0">
                                  <div className="text-left space-y-1 flex-1 min-w-0">
                                    <div className="font-semibold text-sm sm:text-base break-words">{lesson.title}</div>
                                    {lesson.description && (
                              ...
                              <AccordionContent className="px-3 sm:px-6 pb-3 sm:pb-4">
                                <div className="space-y-2">
                                  <div className="flex items-center justify-between p-3 sm:p-4 rounded-lg border border-border/30 bg-background/50 hover:bg-muted/40 transition-colors">
                                    <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                                      {isEnrolled ? (
                                        progress.completedLessonIds.includes(lesson.id) ? (
                                          <CheckCircle className="h-4 w-4 sm:h-5 sm:w-5 text-primary-500 flex-shrink-0" />
                                        ) : (
                                      ...
                                      <div className="space-y-1 flex-1 min-w-0">
                                        <div className="font-medium text-sm sm:text-base break-words">{lesson.title}</div>
                                        {lesson.description && (
                                          <div className="text-xs sm:text-sm text-muted-foreground break-words">
                                            {lesson.description}
                                          </div>
                      ...
                      <TabsContent value="quizzes" className="space-y-4 sm:space-y-6 mt-4 sm:mt-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                          {quizzes.length === 0 ? (
                            <div className="text-center col-span-full text-muted-foreground py-6 sm:py-8">
                            ...
                            quizzes.map((quiz) => (
                              <Card key={quiz.id} className="hover:shadow-lg transition-shadow">
                                <CardHeader className="pb-3">
                                  ...
                                  {quiz.description && <p className="text-xs sm:text-sm text-muted-foreground mt-1 line-clamp-2">{quiz.description}</p>}
                                </CardHeader>
                                <CardContent className="pt-0">
                      ...
                      <TabsContent value="lectures" className="space-y-4 sm:space-y-6 mt-4 sm:mt-6">
                ...
                <CardContent className="pt-0">
                  <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6">
                    ...
                    <div className="flex-1 space-y-3 text-center sm:text-left min-w-0">
                      <div>
                        <h3 className="text-lg sm:text-xl font-semibold break-words">{teacherProfile?.display_name || t('studentCourseView.unknownInstructor')}</h3>
                        {teacherProfile?.specialization && <p className="text-sm sm:text-base text-muted-foreground break-words">{teacherProfile.specialization}</p>}
                        {teacherProfile?.bio && <p className="text-xs sm:text-sm text-muted-foreground mt-1 break-words">{teacherProfile.bio}</p>}
                      </div>
              ...
              <div className="sticky top-6">
                <Card className="glass-card border-white/10 shadow-2xl">
                  <CardContent className="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
                    ...
                    {/* Price */}
                    <div className="text-center space-y-2">
                      <div className="text-2xl sm:text-3xl lg:text-4xl font-bold bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">
                        {course.price} {t('studentCourseView.egp')}
                      </div>
                      <p className="text-xs sm:text-sm text-muted-foreground">{t('studentCourseView.oneTimePurchase')}</p>
                    </div>
                    ...
                    {/* Course Features */}
                    <div className="space-y-3 sm:space-y-4 pt-3 sm:pt-4 border-t border-border/20">
                      <h4 className="font-semibold text-sm sm:text-base">{t('studentCourseView.thisCourseIncludes')}</h4>
                      <div className="space-y-2 sm:space-y-3">
                        {[
                          { icon: PlayCircle, text: `${progress.totalLessons} ${t('studentCourseView.onDemandVideoLessons')}` },
                          { icon: FileText, text: t('studentCourseView.downloadableResources') },
