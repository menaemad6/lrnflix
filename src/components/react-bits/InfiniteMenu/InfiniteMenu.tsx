...
interface MenuItem {
  ...
  title: string;
  description: string;
  fallbackAvatar?: string; // Add fallback avatar path
}
...
class InfiniteGridMenu {
            ...
            if (isExternalUrl) {
              ...
              if (item.fallbackAvatar) {
                ...
                fallbackImg.onerror = (error) => {
                  console.error(`Failed to load avatar fallback for ${item.title}:`, item.fallbackAvatar, 'Error:', error);
                  // If avatar also fails, create colored rectangle as last resort
                  const canvas = document.createElement('canvas');
                  canvas.width = cellSize;
                  canvas.height = cellSize;
                  const fallbackCtx = canvas.getContext('2d')!;
                  fallbackCtx.fillStyle = `hsl(${index * 137.5 % 360}, 70%, 60%)`;
                  fallbackCtx.fillRect(0, 0, cellSize, cellSize);
                  fallbackCtx.fillStyle = 'white';
                  fallbackCtx.font = 'bold 48px Arial';
                  fallbackCtx.textAlign = 'center';
                  fallbackCtx.textBaseline = 'middle';
                  fallbackCtx.fillText(item.title.charAt(0).toUpperCase(), cellSize/2, cellSize/2);
                  
                  const coloredFallbackImg = new Image();
                  coloredFallbackImg.onload = () => resolve(coloredFallbackImg);
                  coloredFallbackImg.src = canvas.toDataURL();
                };
                fallbackImg.src = item.fallbackAvatar;
              } else {
                // No fallback avatar, create colored rectangle
                const canvas = document.createElement('canvas');
                canvas.width = cellSize;
                canvas.height = cellSize;
                const fallbackCtx = canvas.getContext('2d')!;
              ...
              img.onerror = (error) => {
                console.error(`Failed to load local image for item ${index}:`, item.image, 'for item:', item.title, 'Error:', error);
                // Use avatar as fallback for local images too
                if (item.fallbackAvatar) {
                  const fallbackImg = new Image();
                  fallbackImg.onload = () => {
  ...
  private render(): void {
    ...
    gl.clearColor(1, 1, 1, 1); // Changed from black (0,0,0,0) to white (1,1,1,1)
    ...
    <div className="relative w-full h-full">
      ...
      {activeItem && (
        <>
          ...
          font-black
          text-black
          text-2xl md:text-3xl lg:text-4xl xl:[font-size:3.5rem]
          left-1/2 md:left-4 lg:left-8 xl:left-[-0.5em]
          top-[15%] md:top-1/3 lg:top-1/2
          transform
          -translate-x-1/2 md:translate-x-4 lg:translate-x-8 xl:translate-x-[20%]
          -translate-y-1/2
          transition-all
        ...
        `}
          ...
          select-none
          absolute
          max-w-[8ch] md:max-w-[10ch] lg:max-w-[12ch]
          text-black
          font-bold
          text-sm md:text-base lg:text-lg xl:text-[1.5rem]
          top-3/4 md:top-2/3 lg:top-1/2
          right-2 md:right-4 lg:right-8 xl:right-[10%]
          hidden md:block
          transition-all
          ease-[cubic-bezier(0.25,0.1,0.25,1.0)]
          ${
