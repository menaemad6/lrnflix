...
export const IphoneShowcaseSection: React.FC<IphoneShowcaseSectionProps> = ({
  ...
  // Animation direction
  const initialX = imagePosition === "right" ? 400 : -400;
  const isRight = imagePosition === "right";

  return (
    <section
      ref={ref}
      className={`w-full flex ${isRight ? 'flex-row' : 'flex-row-reverse'} items-center justify-between min-h-[20vh] sm:min-h-[50vh] md:min-h-[60vh] lg:min-h-[90vh] xl:min-h-[110vh] px-4 md:px-24 py-12 gap-8 bg-transparent relative overflow-x-visible`}
      style={{ background: "transparent" }}
    >
      <motion.div
        className="flex-1 flex items-center relative  h-full z-0"
        style={{ position: 'absolute', [isRight ? 'right' : 'left']: 0, top: 0, bottom: 0,  pointerEvents: 'none' }}
      ...
      >
        <h1 className="text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-extrabold text-slate-900 dark:text-white leading-tight mb-2">
          {leftTextTop}
        </h1>
        <h2 className="text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-extrabold text-slate-900 dark:text-white leading-tight">
          {leftTextBottom}
        </h2>
      </div>
    </section>
